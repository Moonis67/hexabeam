...
export default function Home(){const schema={"@context":"https://schema.org","@type":"Organization",name:"Hexa Beam",url:"https://hexabeam.ae",areaServed:"United Arab Emirates",description:"Coordinated UAE business, residency, tax, accounting, corporate support, logistics and concierge services."};return <main id="main">
 ...
 <section className="section muted-bg"><div className="container"><div className="section-head"><div><div className="eyebrow">Dubai and UAE questions</div><h2>Clear answers before you take the next step.</h2></div></div><div className="faq">{faqs.map(([q,a])=><details key={q}><summary>{q}</summary><p>{a}</p></details>)}</div><div className="client-paths-cta"><div><b>Looking for a specific service?</b><p>Browse the full range of company setup, residency, tax, accounting and business support services.</p></div><Link className="btn btn-primary" href="/services/">Explore all services <ArrowRight size={16}/></Link></div></div></section>
 <CtaBand/>
 </main>}
